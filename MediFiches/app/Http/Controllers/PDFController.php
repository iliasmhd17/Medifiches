...
class PDFController extends Controller
{
 public function generatePDFs()
    ...
    {
        $data = Infos_Pdf::find(2);
        //$medical_card = MedicalCard::getMedicalCardById($id);
        // if ($medical_card) {
        //     return view('animateur/fiche_medicale', compact('medical_card'));
        // } else {
        //     return redirect()->route('medical_card.index')->with('error', 'Fiche médicale non trouvée.');
        // }
        if ($data) {
            $pdf = PDF::loadView('animateur/fiche_medicale', compact('data'));
